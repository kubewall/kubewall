...
const useDetailsWrapper = ({ loading, resourcekind }: DetailsWapperProps) => {
  ...
  if (resourcekind === HELM_RELEASES_ENDPOINT) {
    return { ...getHelmReleaseDetailsConfig(helmReleaseDetails || { release: {} as any, history: [], values: '', templates: '', manifests: '' }, loading), miscComponent: <HelmReleaseDetailsContainer/> };
  }
};
