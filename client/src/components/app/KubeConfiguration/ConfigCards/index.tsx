              ...
              return (
                <Card key={name} className="flex flex-col bg-muted/30 shadow-none">
                  <CardHeader className="flex-1 flex-row items-start justify-between gap-2 space-y-0 p-4 pb-3">
                    {/* Ties the avatar and name hover states together — hovering
                        either dims the avatar and underlines the name, so the two read
                        as the one target they are.

                        The trigger is `group-has-[[data-cluster-link]:hover]`, not a
                        plain `group-hover`. This wrapper is `flex-1`, so it stretches
                        across the header and down past the namespace line and the addon
                        badges; on `group-hover` all of that dead space lit the pair up
                        and advertised a click that only the avatar and the name
                        actually accept. `:has()` narrows the trigger to the two real
                        targets while keeping them linked — which a `group` per button
                        could not do, since neither is an ancestor of the other. */}
                    <div className="group/link flex min-w-0 flex-1 items-center gap-3">
                      ...
                      {/* The hover cue is carried by the avatar and the name only —
                          not by the card, nor by the `group/link` wrapper, which would
                          pull in the namespace line and the addon badges below it.
                          They're separate elements rather than one shared ancestor
                          because the name sits a level deeper than the avatar, so
                          crossing from one to the other can re-fire; uisfx's 150ms
                          cooldown on `hover` absorbs that at any normal pointer speed.
                          Clicking either sounds "select" via onNavigate, exactly as in
                          the table view — no attribute needed for that. */}
                      <button
                        type="button"
                        tabIndex={-1}
                        aria-hidden
