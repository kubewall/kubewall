...
import { updatePersistentVolumeClaimPods } from "@/data/Storages/PersistentVolumeClaims/PersistentVolumeClaimPodsSlice";
...
import { PERSISTENT_VOLUME_CLAIMS_ENDPOINT, PODS_ENDPOINT } from "@/constants";
import { toast } from "sonner";

const PersistentVolumeClaimDetailsContainer = memo(function () {
  const { config } = appRoute.useParams();
  const { cluster, resourcename, namespace } = kwDetails.useSearch();
  const navigate = useNavigate();
  const {
    loading,
    persistentVolumeClaimPodDetails
  } = useAppSelector((state) => state.persistentVolumeClaimPods);
  const { open } = useSidebar();
  const dispatch = useAppDispatch();

  const sendMessage = (message: Pods[]) => {
    dispatch(updatePersistentVolumeClaimPods(message));
  };
  ...
  const handleViewPods = () => {
    navigate({ 
      to: `/${config}/list?cluster=${encodeURIComponent(cluster)}&resourcekind=${PODS_ENDPOINT}&namespace=${encodeURIComponent(namespace || '')}&owner=persistentvolumeclaim&ownerName=${encodeURIComponent(resourcename)}` 
    });
  };
              ...
              <DataTable
                columns={
                  useGenerateColumns<Pods, HeaderList>({
                    ...
                    count: persistentVolumeClaimPodDetails.length,
                  })
                }
                data={loading ? defaultSkeletonRow() : persistentVolumeClaimPodDetails}
...
export default PersistentVolumeClaimDetailsContainer;