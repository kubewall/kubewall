...
import { statefulSetRestart, resetStatefulSetRestart } from "@/data/Workloads/StatefulSets/StatefulSetRestartSlice";
...
type RestartStatefulSetsProps = {
  resourcename: string;
  queryParams: string;
}

const RestartStatefulSets = ({ resourcename, queryParams }: RestartStatefulSetsProps) => {
  const {
    loading,
    error,
    message
  } = useAppSelector((state: RootState) => state.statefulSetRestart);
  const {
    statefulSetDetails
  ...
  useEffect(() => {
    const checkPermission = async () => {
      ...
      try {
        const qp: Record<string, string> = { config, cluster, resourcekind: 'statefulsets', verb: 'update' };
  ...
  const updateStatefulSetRestart = () => {
    dispatch(statefulSetRestart({
      name: resourcename,
      queryParams,
      restartType: 'rolling'
    }));
  ...
  useEffect(() => {
    if (message) {
      ...
      dispatch(resetStatefulSetRestart());
      resetDialog();
    } else if (error) {
      toast.error("Failure", {
      ...
      <TooltipProvider>
        <Tooltip delayDuration={0}>
          ...
          <TooltipContent side="bottom">
            {checkingPermission ? 'Checking permissions...' : (!canRestart ? "You don't have permission to restart" : 'Restart StatefulSet')}
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Restart StatefulSet</DialogTitle>
          <DialogDescription>
            <div className="mt-2">
              <div className="flex flex-col gap-4 text-sm">
                <p>
                  Are you sure you want to restart the statefulset <strong>{resourcename}</strong>?
                </p>
                <p className="text-muted-foreground">
                  This will perform a rolling restart of all pods in the StatefulSet.
                </p>
              </div>
            </div>
        ...
        <DialogFooter className="sm:justify-center">
          ...
          >Cancel</Button>
          <Button
            onClick={updateStatefulSetRestart}
            className="md:w-2/4 w-full"
            type="submit"
          >Restart</Button>
        </DialogFooter>
